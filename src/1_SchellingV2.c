/**
 * * Schelling_1 V2
 * Differisce dal primo per aver sistemato il codice
 * Es. 1 - La funzione calculate_destination
 * Es. 2 - Le firme dei metodi prima del main
*/
...
// #region Matrice
#define ROWS 60     // Numero di righe della matrice
#define COLUMNS 30  // Numero di colonne della matrice
// #endregion
...
voidCell calculate_destination(int, int **);                                             // Funzione per calcolare la cella di destinazione di un agente
...
void move(int rank, int original_rows, char mat[original_rows][COLUMNS], int **want_move) {
    for (int i = 0; i < original_rows; i++)
        for (int j = 0; j < COLUMNS; j++) {
            ...
            if (*(*(want_move + i) + j) == 1) {
                voidCell destination = calculate_destination(original_rows, want_move);
...
voidCell calculate_destination(int original_rows, int **want_move) {
    ...
    if (ind > 0) {
        ...
        voidCell var = void_cells[rand_pos];
        return var;
    }
